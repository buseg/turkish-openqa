...
class RRAttention(T5Attention):
    ...
    def forward(
        ...
        if self.layer_index > 0 and not self.is_decoder:
        ...
        if not self.is_decoder and self.layer_index < self.config.retriever_layer:
            assert independent_mask is not None
            masked_position_bias = position_bias + independent_mask
        else:
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, layer_index: int = None):
        super().__init__()
        self.SelfAttention = RRAttention(config, has_relative_attention_bias=has_relative_attention_bias,
...
class RRBlock(nn.Module):
    ...
    def forward(
        ...
        return outputs  # hidden-states, present_key_value_states, (self-attention position bias), (self-attention
        # weights), (cross-attention position bias), (cross-attention weights)


class RRStack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        self.block = nn.ModuleList(
            [RRBlock(config, has_relative_attention_bias=bool(i == 0), layer_index=i) for i in range(config.num_layers)]
        )
    ...
    def copy_relative_attention_bias(self):
        if hasattr(self, '_copied') and self._copied:
            return
        rab = self.block[0].layer[0].SelfAttention.relative_attention_bias
        for layer_index, block in enumerate(self.block):
            block.layer[0].SelfAttention.relative_attention_bias = rab
            block.layer[0].SelfAttention.has_relative_attention_bias = True
            block.layer[0].SelfAttention.layer_index = layer_index
    ...
    def forward(
        ...
        extended_independent_mask = None
        if independent_mask is not None:
            extended_independent_mask = self.get_extended_attention_mask(independent_mask, input_shape,
        ...
        checkpointing_layer = list(range(self.config.retriever_layer)) + [13, 14, 15]
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if self.gradient_checkpointing and self.training and requires_grad and i in checkpointing_layer:
                if use_cache:
                    logger.warning(
            ...
            if i >= self.config.retriever_layer and only_encoding:
                break
...
class RRForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        ...
        self.encoder = RRStack(encoder_config, self.shared)

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.is_encoder_decoder = False
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = RRStack(decoder_config, self.shared)
    ...
    def encode(self, inputs, only_encoding=False):
        attention_mask = inputs['attention_mask']
        outputs = self.encoder(**inputs, independent_mask=attention_mask, query_mask=attention_mask,
                               passage_mask=attention_mask, output_attentions=True, return_dict=True,
                               only_encoding=only_encoding)
        query_vector, passage_vector = outputs.attentions[self.config.retriever_layer]

        return query_vector, passage_vector
    ...
    def forward(
            ...
            independent_mask: Optional[torch.FloatTensor] = None,
        ...
        if passage is not None:
            passage_vector = self.encode(passage, only_encoding=only_encoding)[1]

        if only_encoding and (query_vector is not None or passage_vector is not None):
        ...
        if encoder_outputs is None:
            ...
            if decoder_input_ids is not None or (labels is not None and hidden_states.size()[0] != labels.size()[0]):
