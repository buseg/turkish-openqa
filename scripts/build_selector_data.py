...
def tokenize(text):
    normalized = unicodedata.normalize("NFKC", text).lower().replace("ı", "i")
...
def compact_context(example, index=0, chars=220):
    ctxs = example.get("ctxs", [])
    if index >= len(ctxs):
        return {"title": "", "text": ""}
    ctx = ctxs[index]
...
def make_record(original, rewritten):
